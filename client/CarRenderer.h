...
/**
 * @brief Gestisce tutto ciò che riguarda la rappresentazione visiva dell'auto.
 *
 ...
 *  - Non conosce nulla di input né di fisica.
 */
class CarRenderer
{
public:
    /**
     * @brief Inizializza il renderer cercando i nodi nella scena a partire da passedNode.
     * @param passedNode  Qualsiasi nodo della gerarchia di scena.
     * @param outInitPos  [out] Posizione iniziale letta dalla matrice OVO del modello.
     * @return true se il nodo "Car" è stato trovato, false altrimenti.
     */
    bool init(Node* passedNode, double& outPosX, double& outPosY, double& outPosZ);
     ...
     */
    void update(const CarPhysicsState& physics, double steeringAngle, double deltaTime);
...
private:
    ...
    static constexpr const char* WHEEL_NAMES[4] = { "RuotaAD", "RuotaAS", "RuotaPD", "RuotaPS" };
