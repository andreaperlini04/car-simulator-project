...
2000.0;                               // Plane: 2000 m x 2000 m (real world)
...
constexpr double GAME_ACCEL = ACCEL_MS2 / METERS_PER_UNIT;        // ~109  u/s^2
constexpr double GAME_BRAKE = BRAKE_MS2 / METERS_PER_UNIT;        // ~364  u/s^2
constexpr double GAME_FRICTION = FRICTION_MS2 / METERS_PER_UNIT;  // ~19.7 u/s^2
constexpr double GAME_REVERSE_MAX =
REVERSE_MAX_MS / METERS_PER_UNIT; // ~328  u/s

// --- Fixed-timestep loop ---
constexpr double PHYSICS_FIXED_DT = 1.0 / 60.0; // 60 Hz physics tick
constexpr double PHYSICS_MAX_FRAME = 0.25;        // spiral-of-death guard (s)

// --- Steering ---
// Angolo massimo di sterzata in gradi.
// Usato da: CarInputController, CarPhysics, Application (mouse steering).
// Modificare solo qui per cambiare il comportamento in tutti i sistemi.
