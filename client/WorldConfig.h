...
constexpr double PLANE_REAL_SIZE_M     = 2000.0;        // Plane: 2000 m × 2000 m (real world)
...
constexpr double METERS_PER_UNIT       = PLANE_REAL_SIZE_M / PLANE_UNITS;  // ~0.0254 m/unit

// --- Realistic physics parameters (SI: m/s, m/s²) ---
constexpr double MAX_SPEED_KMH         = 100.0;                             // Top speed: 100 km/h
constexpr double MAX_SPEED_MS          = MAX_SPEED_KMH / 3.6;               // = 27.778 m/s
constexpr double ACCEL_MS2             = MAX_SPEED_MS / 10.0;               // 0→100 in ~10 s => 2.778 m/s²
constexpr double BRAKE_MS2             = MAX_SPEED_MS / 3.0;                // 100→0 in ~3 s  => 9.259 m/s²
...
// --- Converted to game units (units/s, units/s²) ---
...
constexpr double GAME_REVERSE_MAX      = REVERSE_MAX_MS / METERS_PER_UNIT;  // ~328  u/s
