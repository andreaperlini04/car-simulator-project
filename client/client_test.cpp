...
void test_CarPhysics() {
   ...
   // Test 3: Tetto massimo della velocit� in avanti
   ...
   physics.update(5.0, input); // 5.0s � > MAX_DELTA_TIME (1.0). Deve fare return.
   ...
   // Acceleriamo per 2 secondi a 10m/s^2 -> velocit� ~20
   ...
   // Sterziamo al massimo per 1 secondo senza gas n� freni
   inScrub.steeringAngle = 35.0;
   physScrub.update(1.0, inScrub);

   // La velocit� DEVE essere diminuita a causa di applyTireScrub()
   ...
   // Acceleriamo sopra al DRIFT_MIN_SPEED (che nel tuo file � 5.0)
   inDrift.isAccelerating = true;
   physDrift.update(2.0, inDrift); // Velocit� ~20.0
   ...
   // 2. La macchina deve aver perso velocit� velocemente per via del "slideDecel"
