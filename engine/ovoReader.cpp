...
// Camera.h include
#include "ovoReader.h"
#include <memory> // Aggiunto per std::shared_ptr e std::unique_ptr
using namespace std;
...
Node ENG_API* OvoReader::readFile(const char* file_path, const char* texture_dir) {
  ...
  //First we parse the material chunks
   ...
   while (isHeader) {
      ...
      int rollback = 0;
      std::shared_ptr<Material> material; // CORRETTO: Uso di shared_ptr
...
// CORRETTO: Restituisce uno shared_ptr
std::shared_ptr<Material> ENG_API OvoReader::parse_material(char* data, unsigned int& position, const char* texture_dir)
{
   ...
   position += sizeof(float);
   std::cout << "[OvoReader] Parsing Material: '" << materialName << "' " << transparency << std::endl;
   // <--- LOG
   ...
   // CORRETTO: Creazione sicura tramite make_shared
   std::shared_ptr<Material> material = std::make_shared<Material>(materialName, emission4, albedo4 * 0.1f, albedo4, specular4, shininess, transparency);
   ...
   if (std::string{ albedoTexture } != "[none]") {
      ...
      std::cout << "   [Texture] Loading Albedo: " << path << std::endl;
      // <--- LOG

      // CORRETTO: Creazione sicura tramite make_shared
      std::shared_ptr<Texture> t = std::make_shared<Texture>(albedoTexture, path);
...
Mesh ENG_API* OvoReader::parse_mesh(char* data, unsigned int& position, unsigned int* n_children)
{
   ...
   *n_children = children;
   ...
   // CORRETTO: Uso delle parentesi () invece che {} per non confondere l'initializer list,
   //           visto che passiamo `material->second` (che ora è correttamente uno shared_ptr).
   ...
   std::cout << "   -> Vertices: " << vertices << ", Faces: " << faces << std::endl;
   // <--- LOG
...
Light ENG_API* OvoReader::parse_light(char* data, unsigned int& position, unsigned int* n_children)
{
   ...
   *n_children = children;
   ...
   std::cout << "[OvoReader] Light found: '" << lightName << "' (" << subtypeName << ")" << std::endl;
   // <--- LOG
